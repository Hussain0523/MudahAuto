...
public class CarServiceImpl implements CarService {
    ...
    public CarServiceImpl(WebClient webClient, GoogleServiceImpl googleService, CarDetailsRepository carDetailsRepository, CarImageRepository carImageRepository) {
                ...
                if (!data.isEmpty()) {
                    ...
                    if (photoUrl != null && !photoUrl.isEmpty()) {
                        try {
                            ...
                            throw new RuntimeException("Error processing photo URL", e);
                        }
                    } else {
                        log.warn("No photo URL found in car data for id: {}", id);
                    }

                    return new CarListingResponse(carData, new ArrayList<>());
                } else {
        ...
        return Mono.fromCallable(() -> {
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    if (carListingResponses.isEmpty()) {
                        log.warn("No car data found in response");
                        throw new RuntimeException("No car data found in response");
                    }

                    log.info("Successfully fetched {} car listings", carListingResponses.size());
